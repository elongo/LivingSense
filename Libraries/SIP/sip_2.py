...
def device_on(sid):
    ...
    GPIO.setup(GPIO_LIGHT,GPIO.OUT)
    GPIO.setup(GPIO_FAN,GPIO.OUT)
    print "sid in device_ON =", sid
    if sid ==  0:
        tRF2.transmit_code(a_on) # turn on pump by Radio Frequency
        print "PUMP ON"
        time.sleep(0.1)
    elif sid == 1:
...
def device_off(sid):
    ...
    GPIO.setup(GPIO_LIGHT,GPIO.OUT)
    GPIO.setup(GPIO_FAN,GPIO.OUT)
    print "sid in device_OFF", sid
    if sid ==  0:
        tRF2.transmit_code(a_off) # turn on pump by Radio Frequency
        print "PUMP OFF"
        time.sleep(3)
    elif sid == 1:
        GPIO.output(GPIO_LIGHT, True)
